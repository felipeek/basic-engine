...
static void physics_update(Entity* cube, r32 dt) {
	...
	//rotationQuat.x = angular_velocity.x * dt;
	//rotationQuat.y = angular_velocity.y * dt;
	//rotationQuat.z = angular_velocity.z * dt;
	//rotationQuat.w = 0.0f;
	//Quaternion res = quaternion_product(&rotationQuat, &cube->world_rotation);
	//Quaternion new_rotation = cube->world_rotation;
	//new_rotation.x += 0.5f * rotationQuat.x;
	//new_rotation.y += 0.5f * rotationQuat.y;
	//new_rotation.z += 0.5f * rotationQuat.z;
	//new_rotation.w += 0.5f * rotationQuat.w;
	//graphics_entity_set_rotation(cube, quaternion_normalize(&new_rotation));
	#else
	r32 angular_velocity_length = dt * gm_vec3_length(angular_velocity) * (57.295779513f);
	Quaternion orientation_change = quaternion_new(gm_vec3_normalize(angular_velocity), angular_velocity_length);
	graphics_entity_set_rotation(cube, quaternion_product(&cube->world_rotation, &orientation_change));
	#endif
    // Generate rotation matrix from angular velocity
    r32 _angle = gm_vec3_length(angular_velocity) * dt;
    vec3 _axis = gm_vec3_normalize(angular_velocity);
    mat3 _rot = rotation_matrix_from_axis_and_angle(_axis, _angle);
	mat4 _m = gm_mat3_to_mat4(&_rot);
	Quaternion _q = quaternion_from_matrix(&_m);
	Quaternion res = quaternion_product(&_q, &cube->world_rotation);
	graphics_entity_set_rotation(cube, res);
...
void physics_simulate(Entity* cube, Entity* plane, r32 plane_y, r32 dt, Physics_Force* forces) {
	...
	//    dt_fact /= 2
	//    restore bodies
	//    store collided_vertex
	// else if (collided_vertex exists)
	//    apply impulse to vertex position [note that this will never be under the plane!];
	// else
	//    break
	...
	for (u32 i = 0; i < 10; ++i) {
		...
		if (array_get_length(collision_points) > 0) {
			...
			for (u32 j = 0; j < array_get_length(collision_points); ++j) {
				...
				apply_impulse(cube, plane, &collision_points[j], 0, 0.2f);
			}
